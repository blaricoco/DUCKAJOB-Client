...
export function useJobContract(contract:string ) {
    ...
    const jwContract = useAsyncInitialize(async () => {
      ...
      return client!.open(
        new JobContract(Address.parse(wallet!))
      ) as OpenedContract<JobContract>;
    }, [jobContract , client]);
    ...
    const { data, isFetching } = useQuery(
        ...
        async () => {
            ...
            const test = await jobContract?.send(sender, { value: toNano(1) },  { $$type: "Fund_Project", amount: 250n })
            console.log(test);
            console.log(await jobContract?.getContractStatus());
